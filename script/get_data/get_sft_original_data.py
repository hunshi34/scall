...
sys.path.append(os.path.abspath(os.path.join('/public/home/cit_xiaominyu/single-cell', 'model', 'scGPT_main')))

from scgpt.tokenizer import GeneVocab


from model.scGPT_main.scgpt.tokenizer import GeneVocab
...
def make_pretrain_id_name_set(data, revocab):
    ...
    for idx in tqdm(range(len(data))):
        ...
        for key in data[idx].obs.keys():
            cell_data[key] = str(data[idx].obs[key])
...
for i, adata in enumerate(adatas):
    dataset = make_pretrain_id_name_set(adata, re_vocab)
